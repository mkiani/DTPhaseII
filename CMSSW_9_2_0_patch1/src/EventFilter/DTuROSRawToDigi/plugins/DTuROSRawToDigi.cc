...
//----- BILAL Addition --- // 
        ...
        {
        int tpf   = ( dataWord >> 32 ) & 0x1F; // positions  32 -> 36           
        int BX   = ( dataWord >> 37 ) & 0xFFF; // positions  37 -> 48

        float fL1TrigTime = ( BX * 25 ) + ( tpf * 25 / 30.0 ) ;
        std::cout << "This should be floating point "<< fL1TrigTime - bxCounter << std::endl;
printf("BX Counter, BX %d %d\n", bxCounter,  BX);
